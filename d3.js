d3 = function() {
  var MathPI = Math.PI, _1e6 = 1e-6, d3 = {
    version: "3.0.8"
  }, d3_radians = MathPI / 180, d3_degrees = 180 / MathPI, d3_document = document, d3_window = window;
  ...
  function d3_ease_elastic(a, p) {
    ...
    if (arguments.length) s = p / (2 * MathPI) * Math.asin(1 / a); else a = 1, s = p / 4;
    return function(t) {
      return 1 + a * Math.pow(2, 10 * -t) * Math.sin((t - s) * 2 * MathPI / p);
    };
  }
  ...
  function d3_lab_hcl(l, a, b) {
    return d3_hcl(Math.atan2(b, a) / MathPI * 180, Math.sqrt(a * a + b * b), l);
  }
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      a0 = a1, a1 = da), a1 - a0), df = da < MathPI ? "0" : "1", c0 = Math.cos(a0), s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
  ...
  var d3_svg_arcOffset = -MathPI / 2, d3_svg_arcMax = 2 * MathPI - 1e-6;
  ...
  d3.layout.pie = function() {
    var value = Number, sort = d3_layout_pieSortByValue, startAngle = 0, endAngle = 2 * MathPI;
    function pie(data) {
  ...
  function d3_geo_sphericalEqual(a, b) {
    return Math.abs(a[0] - b[0]) < _1e6 && Math.abs(a[1] - b[1]) < _1e6;
  }
  ...
  function d3_geo_resample(project) {
    ...
    function resampleLineTo(x0, y0, Î»0, a0, b0, c0, x1, y1, Î»1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * Î´2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), Ï†2 = Math.asin(c /= m), Î»2 = Math.abs(Math.abs(c) - 1) < _1e6 ? (Î»0 + Î»1) / 2 : Math.atan2(b, a), p = project(Î»2, Ï†2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
  ...
  function d3_geo_albers(Ï†0, Ï†1) {
    ...
    albers.invert = function(x, y) {
      var Ï0_y = Ï0 - y;
      return [ Math.atan2(x, Ï0_y) / n, Math.asin((C - (x * x + Ï0_y * Ï0_y) * n * n) / (2 * n)) ];
    };
    return albers;
  }
  ...
  d3.geo.centroid = function(object) {
    ...
    var m;
    if (d3_geo_centroidW && Math.abs(m = Math.sqrt(d3_geo_centroidX * d3_geo_centroidX + d3_geo_centroidY * d3_geo_centroidY + d3_geo_centroidZ * d3_geo_centroidZ)) > _1e6) {
  ...
  function d3_geo_circleInterpolate(radians, precision) {
    ...
    return function(from, to, direction, listener) {
      if (from != null) {
        ...
        if (direction > 0 ? from < to : from > to) from += direction * 2 * MathPI;
      } else {
        from = radians + direction * 2 * MathPI;
        to = radians;
      }
      var point;
  ...
  function d3_geo_circleAngle(cr, point) {
    ...
    return ((-a[2] < 0 ? -angle : angle) + 2 * Math.PI - _1e6) % (2 * Math.PI);
  }
    ...
    return function(listener) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          } else if (visibleArea < -_1e6 || invisible && invisibleArea < -_1e6) {
            listener.lineStart();
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.point)[0] < 0 ? a[1] - MathPI / 2 - _1e6 : MathPI / 2 - a[1]) - ((b = b.point)[0] < 0 ? b[1] - MathPI / 2 - _1e6 : MathPI / 2 - b[1]);
  }
  ...
  function d3_geo_clipAreaRing(ring, invisible) {
    ...
    while (++i < n) {
      ...
      if (Math.abs(y0 - 2) < _1e6 && Math.abs(y - 2) < _1e6) continue;
      if (Math.abs(y) < _1e6 || Math.abs(y0) < _1e6) {} else if (Math.abs(Math.abs(x - x0) - MathPI) < _1e6) {
        if (y + y0 > 2) area += 4 * (x - x0);
      } else if (Math.abs(y0 - 2) < _1e6) area += 4 * (x - x1); else area += ((3 * MathPI + x - x0) % (2 * MathPI) - MathPI) * (y0 + y);
      x1 = x0, x0 = x, y0 = y;
    }
    return area;
  }
  ...
  function d3_geo_clipAntimeridianLine(listener) {
    ...
    return {
      ...
      point: function(Î»1, Ï†1) {
        var sÎ»1 = Î»1 > 0 ? MathPI : -MathPI, dÎ» = Math.abs(Î»1 - Î»0);
        if (Math.abs(dÎ» - MathPI) < _1e6) {
          listener.point(Î»0, Ï†0 = (Ï†0 + Ï†1) / 2 > 0 ? MathPI / 2 : -MathPI / 2);
          ...
          clean = 0;
        } else if (sÎ»0 !== sÎ»1 && dÎ» >= MathPI) {
          if (Math.abs(Î»0 - sÎ»0) < _1e6) Î»0 -= sÎ»0 * _1e6;
          if (Math.abs(Î»1 - sÎ»1) < _1e6) Î»1 -= sÎ»1 * _1e6;
  ...
  function d3_geo_clipAntimeridianIntersect(Î»0, Ï†0, Î»1, Ï†1) {
    ...
    return Math.abs(sinÎ»0_Î»1) > _1e6 ? Math.atan((Math.sin(Ï†0) * (cosÏ†1 = Math.cos(Ï†1)) * Math.sin(Î»1) - Math.sin(Ï†1) * (cosÏ†0 = Math.cos(Ï†0)) * Math.sin(Î»0)) / (cosÏ†0 * cosÏ†1 * sinÎ»0_Î»1)) : (Ï†0 + Ï†1) / 2;
  }
  function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
    var Ï†;
    if (from == null) {
      Ï† = direction * MathPI / 2;
      listener.point(-MathPI, Ï†);
      listener.point(0, Ï†);
      listener.point(MathPI, Ï†);
      listener.point(MathPI, 0);
      listener.point(MathPI, -Ï†);
      listener.point(0, -Ï†);
      listener.point(-MathPI, -Ï†);
      listener.point(-MathPI, 0);
      listener.point(-MathPI, Ï†);
    } else if (Math.abs(from[0] - to[0]) > _1e6) {
      var s = (from[0] < to[0] ? 1 : -1) * MathPI;
      Ï† = direction * s / 2;
      listener.point(-s, Ï†);
      listener.point(0, Ï†);
      listener.point(s, Ï†);
  ...
  (d3.geo.equirectangular = function() {
    return d3_geo_projection(d3_geo_equirectangular).scale(250 / MathPI);
  ...
  d3.geo.graticule = function() {
    ...
    return graticule.extent([ [ -180 + _1e6, -90 + _1e6 ], [ 180 - _1e6, 90 - _1e6 ] ]);
  };
  function d3_geo_graticuleX(y0, y1, dy) {
    var y = d3.range(y0, y1 - _1e6, dy).concat(y1);
    return function(x) {
  ...
  function d3_geo_graticuleY(x0, x1, dx) {
    var x = d3.range(x0, x1 - _1e6, dx).concat(x1);
    return function(y) {
  ...
  function d3_geo_mercator(Î», Ï†) {
    return [ Î» / (2 * MathPI), Math.max(-.5, Math.min(+.5, Math.log(Math.tan(MathPI / 4 + Ï† / 2)) / (2 * MathPI))) ];
  }
  d3_geo_mercator.invert = function(x, y) {
    return [ 2 * MathPI * x, 2 * Math.atan(Math.exp(2 * MathPI * y)) - MathPI / 2 ];
  };
  ...
  function d3_geo_pathContext(context) {
    ...
    function point(x, y) {
      context.moveTo(x, y);
      context.arc(x, y, pointRadius, 0, 2 * MathPI);
    }
    function pointLineStart(x, y) {
      context.moveTo(x, y);
      stream.point = pointLine;
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function() {
      ...
      d3_geo_areaSum += area < 0 ? 4 * MathPI + area : area;
  ...
  function d3_geo_areaRingStart() {
    ...
    d3_geo_area.point = function(Î», Ï†) {
      ...
      Î»0 = (Î»00 = Î») * d3_radians, cosÏ†0 = Math.cos(Ï† = (Ï†00 = Ï†) * d3_radians / 2 + MathPI / 4),
      sinÏ†0 = Math.sin(Ï†);
    };
    function nextPoint(Î», Ï†) {
      Î» *= d3_radians;
      Ï† = Ï† * d3_radians / 2 + MathPI / 4;
  ...
  function d3_geo_projectionRadiansRotate(rotate, stream) {
    return {
      point: function(x, y) {
        ...
        stream.point(x > MathPI ? x - 2 * MathPI : x < -MathPI ? x + 2 * MathPI : x, y[1]);
      },
      sphere: function() {
        stream.sphere();
      },
  ...
  function d3_geo_forwardRotationÎ»(Î´Î») {
    return function(Î», Ï†) {
      return Î» += Î´Î», [ Î» > MathPI ? Î» - 2 * MathPI : Î» < -MathPI ? Î» + 2 * MathPI : Î», Ï† ];
    };
  }
  ...
  d3.geom.voronoi = function(vertices) {
    ...
    polygons = polygons.map(function(polygon, i) {
      ...
      return order.filter(function(d, i) {
        return !i || angle[d] - angle[order[i - 1]] > _1e6;
      }).map(function(d) {
        return polygon[d];
      });
    });
